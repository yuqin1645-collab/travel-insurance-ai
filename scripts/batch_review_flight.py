...
"""
航班延误批量审核脚本
- 对 claims_data/航班延误/ 下所有案件跑 AI 审核
- 审核结果保存到 review_results/flight_delay/
- 完成后输出与人工审核结论的对比报告

用法:
  python scripts/batch_review_flight.py               # 跑全部
  python scripts/batch_review_flight.py --limit 10   # 只跑前N个
  python scripts/batch_review_flight.py --report-only # 只生成报告（不重跑已有结果）
  python scripts/batch_review_flight.py --rerun-all   # 重跑所有（含已有结果）
"""
...
CLAIMS_DIR = config.CLAIMS_DATA_DIR / "航班延误"
REVIEW_DIR = config.REVIEW_RESULTS_DIR / "flight_delay"
...
def map_ai_verdict(result: dict) -> str:
    ...
    if "拒赔" in remark or "拒绝" in remark:
        return "reject"
    if "需补" in remark or "补件" in remark:
        return "supplement"
    return "unknown"
...
def print_report(outcomes: list):
    ...
    if mismatch_list:
        ...
        for o in mismatch_list:
            ...
            audit = o["result"].get("flight_delay_audit") or {}
            remark = str(o["result"].get("Remark") or "")[:100]
            key = audit.get("key_data") or {}
            print(f"\n  forceid: {o['forceid']}")
            print(f"  人工: {h_raw} -> {h}  |  AI: {o['ai_verdict']}")
    ...
    report_file = Path("review_results/flight_delay_comparison_report.txt")
    try:
        lines = []
        lines.append(f"航班延误 AI vs 人工审核对比报告 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
...
async def main(limit: int = 0, report_only: bool = False, skip_existing: bool = True):
    ...
    print(f"航班延误案件总数: {len(cases)}")

    try:
        terms_file = POLICY_TERMS.resolve("flight_delay")
