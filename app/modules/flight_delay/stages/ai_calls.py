"""
flight_delay AI 调用层 — 从 AIClaimReviewer 提取的独立函数。
...
async def run_flight_delay_parse(
...
async def run_flight_delay_vision_extract(
    vision_client: Any,
    prompt_loader: PromptLoader,
    namespace: str,
...
async def run_flight_delay_audit(
    client: OpenRouterClient,
    prompt_loader: PromptLoader,
    namespace: str,
    claim_info: Dict[str, Any],
    ...
    session: aiohttp.ClientSession,
    payout_json: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    prompt = prompt_loader.format(
        "02_audit_decision",
        namespace=namespace,
